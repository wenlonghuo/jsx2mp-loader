...
var _require5 = require('./utils/judgeModule'),
    isTypescriptFile = _require5.isTypescriptFile;
...
module.exports = /*#__PURE__*/function () {
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            isCustomComponent = function _isCustomComponent(name, usingComponents) {
              ...
              for (var key in usingComponents) {
                if (usingComponents.hasOwnProperty(key) && usingComponents[key] && usingComponents[key].indexOf(matchingPath) === 0) {
                  return true;
                }
              }
            ...
            query = parse(this.request); // Only handle component role file
          ...
          case 32:
            ...
            config = Object.assign({}, transformed.config);
            ...
            distFileDir = dirname(distFileWithoutExt);
            if (!existsSync(distFileDir)) mkdirpSync(distFileDir); // Only works when developing miniapp plugin, to declare the use of __app_css component

            if (injectAppCssComponent) {
              appCssComponentPath = resolve(outputPath, '__app_css', 'index');
              relativeAppCssComponentPath = relative(distFileDir, appCssComponentPath);
            ...
            return _context.abrupt("return", ["/* Generated by JSX2MP ComponentLoader, sourceFile: " + this.resourcePath + ". */", generateDependencies(dependencies)].join('\n'));
  ...
  function componentLoader(_x) {
    return _componentLoader.apply(this, arguments);
  }
